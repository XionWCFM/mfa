...
const http = {
  ...
  delete: function kydelete<Response = unknown>(url: string): Promise<Response> {
    return ky.delete(`${url}`).then((response) => response.json());
  },
  patch: function patch<Request = any, Response = unknown>(url: string, payload?: Request): Promise<Response> {
    return ky.patch(`${url}`, { json: payload }).then((response) => response.json());
  },
};
